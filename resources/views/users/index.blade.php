    ...
    @endforeach    
        ...
        <div class="stage">
            <form action="{{ route('user_create_index') }}" method = "post">
                @csrf
                <button id="create">STAGEの作成</button>
            </form>

            <form action="{{ route('user_edit_index') }}" method = "post">
                @csrf
                <input  type="hidden"  name="title" id="text1" placeholder="タイトル" value="{{ $_POST['title'] }}" >
                <button id="edit">STAGEの編集</button>
            </form>
                    
            <form action="{{ route('contents_delete') }}" method = "post">
                @csrf
                <input  type="hidden"  name="title" id="text1" placeholder="タイトル" value="{{ $_POST['title'] }}" >
                <button id = "delete">STAGEの削除</button>
            </form>
        </div>
